...
- 三形態に同じコアを載せる：
  ...
  3. **ブラウザ WASM**（ローカル観戦とデバッグ）— 実装済み（`crates/web` + `pages/static/play/`）。ブラウザは component model をネイティブ実行できないため、brain / scenario component は jco transpile した core wasm + JS glue で接続する。JS glue が wasmtime Linker 相当（commit 収集・decide ごとの新規インスタンス化）。公式ランは常にネイティブ形態。
     - **ブラウザ fuel は命令数計装で実装済み**（`crates/meter`）: brain の core wasm に「各命令列の先頭で import した mutable global `zv.fuel` を命令数ぶん減算し、負なら trap」を差し込む。ビルトイン brain はビルド時（build-web.sh）、アップロード brain はブラウザ内で計装する。glue が decide 前に予算（health × fuel-per-health）をセットし、後に残量を読んで `Decision.fuel_used` に渡す。**無限ループはネイティブ同様、予算切れ trap（部分実行）で決定論的に停止する**（watchdog は glue 側の異常専用のバックストップに後退）。
     - 計装 fuel は決定論的だが、命令列に入った時点の前払い課金（br で抜けても課金）のため wasmtime の計量とはスケールが完全一致しない。ネイティブとの hash 一致は従来どおり fuel が health 量子（fuel-per-health = 100 万）未満の域に限られる。scenario はメタ層なので計装しない（ネイティブも固定の大予算）。
