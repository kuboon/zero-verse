...
section: 世界の仕様
order: 4
summary: 教育の仕様・teach/learn の WIT
---
...
- **食事** = 食べ物 resource を health と**廃棄物 resource** に変換する skill（廃棄物は discard するまで保有し続ける → [02-resources.md](./resources.md)）

つまり train / harvest は専用アクションではなく skill の一種であり、専用 act は廃止。skill の発動は単一アクション **invoke**（craft 改め → [human.md](./human.md)）に統一する。

health を回復できる skill × resource のペアは**複数**あり、どのペアを使うかが民族間の文化・料理法の違いに相当する。world ごとに法則グラフが違う（[02-resources.md](./resources.md)）のと同じ構図で、同じ world 内でも「食文化」が分岐しうる。
...
- **研鑽（既習者への再教育）**: 初回の習得は初期熟練（`learn-initial-prof`、既定 50%）で完了するが、教育を続ければ**教師の熟練を上限に**上積みできる（1 コースごとに +50%、上限 = 教師の水準）。師を超えることは教育ではできない。世代を跨いで技能水準を維持するには継続教育が必要になる（初取得の 50% で止まった第二世代は生産が薄く、求愛・給餌の余剰が作れないことが M4 派生実験で示された）。
...
支払い側は standing order の `if-taught-me` 条件で月単位アトミックにできる（型定義は [04-market.md](./market.md)）：
