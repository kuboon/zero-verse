...
## M2″：商圏の能動拡大（OTC + mingle）

M2′ の限界（収束の頭打ち・人口減での通貨再分裂）は**知人獲得の受動性**にある — introduce は共通の知人がいる相手にしか届かず（連結成分の内側を密にするだけ）、ε は運任せ。act `mingle`（出歩く。[公理 6](./axioms.md)）の導入で、商人が「生存知人が目標（8 人）に満たない偶数月は、採取を 1 回休んで市に出る」だけの規則を足した。世界は M2′ と同一。

**結果**（20 商人・20 年・5 シード、`mingle_widens_otc_convergence`。出生ノイズを分離するため不妊化した世界で比較）:

- 支払い集中度は 5 シード平均で **390‰ → 554‰**。個々のシードでは 4 勝 1 敗（42→69%、45→51%、54→70%、退化シードの救済 21→65%。1 シードのみ 33→22% に悪化）で、**平均は上がるが分散も増える**
- M2′ で「孤立から抜けられず非最良 λ に固着」していた退化シードが、mingle 版では全シードで**最良 λ（=0）の媒介に収束**するようになった — 橋を架ける手段があると Menger 選別が働き直す
- それでも板の 1000‰ には届かない。**板の固有機能（公開情報による全員一斉の収束）は商圏拡大では代替できない**という M2′ の結論は変わらない

**副作用の発見（出生あり世界）**: mingle で商圏を広げるほど**出生が減り、人口崩壊が早まる**。機構は支払い give の親密度が薄く広く配られ、どのペアも conceive の相対親密度閾値（[05-kinship.md](./kinship.md)）を超えなくなるため — **商業の拡大が婚姻市場を希薄化する**という創発で、実験の比較を交絡させるので上記は不妊化世界で測った。商人 brain が「商圏と家庭の act 配分」を最適化する余地として残る。
