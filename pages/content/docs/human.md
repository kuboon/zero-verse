...
| **fertility**（生殖） | conceive 自動発生の条件の一つ（窓内でのみ発生） | 年齢窓（思春期に開き、加齢で閉じる） | 不可 |
...
- **相対親密度**：self の全知人への親密度合計に占める、特定の相手の割合。親密度の絶対値が同じでも相対親密度は異なる。conceive の自動発生条件に使う（[05-kinship.md](./kinship.md)）。
