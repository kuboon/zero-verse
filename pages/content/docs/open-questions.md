...
8. **時代プリセットの構成**：world パラメータの束（板・ε・σ・寿命・環境・教育/RE）は実装済み（[world.md](./world.md) の表、`Era` / `WorldParams::for_era`）。残る未決は「概念を知っている」の実装形式（初期 memory 接種のフォーマット、brain 側の互換性）と初期 skill 分布の深さ（[10-ideas.md](./ideas.md)）。
