...
section: brain を書く
order: 1
summary: brain ⇔ engine 接続仕様。型定義の分散マップ・不変の原則
---
...
| `types`（resource-id, qty, resource-stack）、`invoke-args`、harvest / give / discard | [02-resources.md](./resources.md) |
| `types`（skill-id）、`skill-view`、`teach-args` / `learn-args` | [03-skills.md](./skills.md) |
| `standing-order`、`limit-order`、`cond-give`、`give-condition`、`board-quote` | [04-market.md](./market.md) |
| `conceive`、出産の観測非対称 | [05-kinship.md](./kinship.md) |
| 通信の搬送路（専用型なし）、約束機構、`proposal` の要否 | [06-communication.md](./communication.md) |
...
| `proposal` | **削除**（無料シグナル化するため → [06-communication.md](./communication.md)） |
