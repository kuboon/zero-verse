...
export function instantiate(getCoreModule, imports, instantiateCore = (module, importObject) => new WebAssembly.Instance(module, importObject)) {
  ...
  const module0 = getCoreModule('scenario-m1.core.wasm');
  ...
  let gen = (function* _initGenerator () {
    ...
    let scenarioApi010Init;
    
    function init(arg0) {
      _debugLog('[iface="zeroverse:scenario/scenario-api@0.1.0", function="init"][Instruction::CallWasm] enter', {
        funcName: 'init',
        paramCount: 1,
        async: false,
        postReturn: true,
      ...
      const [task, _wasm_call_currentTaskID] = createNewCurrentTask({
        ...
        isAsync: false,
        isManualAsync: false,
        entryFnName: 'scenarioApi010Init',
        getCallbackFn: () => null,
        callbackFnName: null,
        errHandling: 'none',
      ...
      try {
        ret =   _withGlobalCurrentTaskMeta({
          taskID: task.id(),
          componentIdx: task.componentIdx(),
          fn: () => scenarioApi010Init(toUint64(arg0)),
        });
      } catch (err) {
      ...
      for (let i = 0; i < len2; i++) {
        const base = base2 + i * 20;
        var len0 = dataView(memory0).getUint32(base + 8, true);
        var base0 = dataView(memory0).getUint32(base + 4, true);
        var result0 = [];
        for (let i = 0; i < len0; i++) {
          const base = base0 + i * 16;
          result0.push({
            skillIndex: dataView(memory0).getInt32(base + 0, true) >>> 0,
            proficiency: BigInt.asUintN(64, BigInt(dataView(memory0).getBigInt64(base + 8, true))),
          });
        }
        ...
        result2.push({
          brainGroup: dataView(memory0).getInt32(base + 0, true) >>> 0,
          skills: result0,
          acquaintances: result1,
        });
      }
      _debugLog('[iface="zeroverse:scenario/scenario-api@0.1.0", function="init"][Instruction::Return]', {
        funcName: 'init',
        paramCount: 1,
        async: false,
        postReturn: true
    ...
    let scenarioApi010Judge;
    
    function judge(arg0) {
      var {month: v0_0, groups: v0_1 } = arg0;
      var vec2 = v0_1;
      var len2 = vec2.length;
      var result2 = realloc0(0, 0, 8, len2 * 24);
      for (let i = 0; i < vec2.length; i++) {
        const e = vec2[i];
        const base = result2 + i * 24;var {group: v1_0, alive: v1_1, total: v1_2, meanConsumed: v1_3 } = e;
        dataView(memory0).setInt32(base + 0, toUint32(v1_0), true);
        dataView(memory0).setInt32(base + 4, toUint32(v1_1), true);
        dataView(memory0).setInt32(base + 8, toUint32(v1_2), true);
        dataView(memory0).setBigInt64(base + 16, toUint64(v1_3), true);
      }
      _debugLog('[iface="zeroverse:scenario/scenario-api@0.1.0", function="judge"][Instruction::CallWasm] enter', {
        funcName: 'judge',
        paramCount: 3,
        async: false,
        postReturn: true,
      ...
      const [task, _wasm_call_currentTaskID] = createNewCurrentTask({
        ...
        isAsync: false,
        isManualAsync: false,
        entryFnName: 'scenarioApi010Judge',
        getCallbackFn: () => null,
        callbackFnName: null,
        errHandling: 'none',
        callingWasmExport: true,
      ...
      try {
        ret =   _withGlobalCurrentTaskMeta({
          taskID: task.id(),
          componentIdx: task.componentIdx(),
          fn: () => scenarioApi010Judge(toUint32(v0_0), result2, len2),
        });
      } catch (err) {
      ...
      var bool3 = dataView(memory0).getUint8(ret + 0, true);
      ...
      _debugLog('[iface="zeroverse:scenario/scenario-api@0.1.0", function="judge"][Instruction::Return]', {
        funcName: 'judge',
        paramCount: 1,
        async: false,
        postReturn: true
      ...
      task.resolve([{
        cleared: bool3 == 0 ? false : (bool3 == 1 ? true : throwInvalidBool()),
        score: BigInt.asUintN(64, BigInt(dataView(memory0).getBigInt64(ret + 8, true))),
        note: result4,
      }]);
      const retCopy = {
        cleared: bool3 == 0 ? false : (bool3 == 1 ? true : throwInvalidBool()),
    ...
    postReturn0 = exports0['cabi_post_zeroverse:scenario/scenario-api@0.1.0#init'];
    
    try {
      postReturn0Async = WebAssembly.promising(exports0['cabi_post_zeroverse:scenario/scenario-api@0.1.0#init']);
    } catch(err) {
      postReturn0Async = exports0['cabi_post_zeroverse:scenario/scenario-api@0.1.0#init'];
    }
    ...
    postReturn1 = exports0['cabi_post_zeroverse:scenario/scenario-api@0.1.0#judge'];
    
    try {
      postReturn1Async = WebAssembly.promising(exports0['cabi_post_zeroverse:scenario/scenario-api@0.1.0#judge']);
    } catch(err) {
      postReturn1Async = exports0['cabi_post_zeroverse:scenario/scenario-api@0.1.0#judge'];
    }
    
    scenarioApi010Init = exports0['zeroverse:scenario/scenario-api@0.1.0#init'];
    scenarioApi010Judge = exports0['zeroverse:scenario/scenario-api@0.1.0#judge'];
    const scenarioApi010 = {
      init: init,
      judge: judge,
      
    };
    
    return { scenarioApi: scenarioApi010, 'zeroverse:scenario/scenario-api@0.1.0': scenarioApi010,  };
  })();
