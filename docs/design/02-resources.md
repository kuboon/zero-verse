...
- 各 resource に隠れた**組成ベクトル**を持たせ、全変換で保存を強制する。組成は world 全体（human の保有 + 環境ストック）で閉じて保存される：消費された組成は環境に還り、harvest で環境から出てくる（[公理 4](./01-axioms.md)、[world.md](./world.md)）。
...
harvest（環境ストックからの採取。獲得量はストック残量に依存）や食事（health 回復。組成は環境へ還元）も専用アクションではなく skill の発動である（[03-skills.md](./03-skills.md)、[world.md](./world.md)）。
...
- [x] 組成保存と消費の整合：**環境還元で解決**。消費で消えた resource の組成は環境ストックに還り、world 全体では厳密に保存される（[world.md](./world.md)）。
