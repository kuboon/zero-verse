...
7. ~~**if-intimacy（担保型の約束）を give-condition に足すか**~~ → **M4 派生実験で判断: 導入見送り**。婚姻契約（相互の贈与均衡）は執行機構なしの繰り返しゲーム（許し付きしっぺ返し + 年次の和解）として維持され、貞節ペアだけが子を残した（浮気者は相対親密度 50% を超えられない）。world に契約執行を入れる必要が現時点で示されなかった。深い契約（多期の債務など）が必要になったら再検討（[06-communication.md](./06-communication.md)、docs/PLAN.md M4 状態）。
