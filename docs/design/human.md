...
1. **出生**：conceive は相対親密度の条件が揃うと自動発生する（[05-kinship.md](./05-kinship.md)）。誕生時に sex が 1/2 で決まる（下記）。
...
- train 系 skill で上げ、老化で下がる。移転不可・非保存（[保存クラス](./01-axioms.md)）。
...
| --- | --- | --- | --- |
| **health**（体力） | 生存そのもの。0 で死。**何もしなければ毎月減少**し、出産・思考（fuel）・空間の占有維持費でも低下する | 最大値が加齢で単調に低下 | resource を消費する skill（食事）で回復（[03-skills.md](./03-skills.md)） |
...
train / harvest は専用アクションではなく **skill**（[03-skills.md](./03-skills.md)）。stats の上げ下げはすべて「skill の発動（invoke）による変換」と「加齢・自然減少」の二経路に統一される。
...
- 帰結 1：見かけ年齢は stats の**正直なシグナル**になる。若く見せるには train 系 skill と食事の実コストがかかるため、Zahavi 原則（[06-communication.md](./06-communication.md)）と整合する。配偶者選択・教師選択の手がかりはここに乗る。
...
- conceive は異性ペアでのみ自動発生する（[05-kinship.md](./05-kinship.md)）。発生しない理由は通知されないので、相手の sex は「条件が揃っているはずなのに conceive が起きない」ことからの推定を含め、確率的にしか判らない（同性・fertility 窓外・刷り込み・親密度の分散と区別がつかない）。
...
- **増減**：毎月、両者の stats に応じた自然増減が起きる。加えて、当事者間で action のやり取り（give / teach / learn / 板での約定 / introduce など）があるたびに増減する。具体式は未決（[90-open-questions.md](./90-open-questions.md) #6）。
- **初期値**：出生時、母子ペアには高い初期値が与えられる。それ以外のペアは 0 から始まる。relation-hint の廃止後、親子の認識はこの初期値と養育の履歴に一本化される。
- **相対親密度**：self の全知人への親密度合計に占める、特定の相手の割合。親密度の絶対値が同じでも相対親密度は異なる。conceive の自動発生条件に使う（[05-kinship.md](./05-kinship.md)）。
...
- decide の WASM fuel 消費は、その human の **health の減少**に写像される（food 概念の廃止に伴う置き換え。[90-open-questions.md](./90-open-questions.md) #11）。思考も身体を使う。重い brain は自分の体力で遅さを払う（[10-ideas.md](./10-ideas.md)）。
...
human は world の有限な空間を占有する（[公理 11](./01-axioms.md)、[world.md](./world.md)）。
...
- 食事や劣化（[02-resources.md](./02-resources.md) の自発変換）は廃棄物 resource を生み、**discard するまで空間を圧迫し維持費を発生させ続ける**。ゴミ処理が生活コストになる。
...
**初回 decide のブートストラップ**：world はプレイヤー参加前から存在している。6歳で brain を引き継いだ直後の decide が受け取る events には、baby brain が先月行った食事（skill 発動で resource が減り health が回復した）の結果が含まれる。brain はここから**どの resource × skill が食事に相当するか**を学ぶ。food を特別扱いする id や設定は存在しない（[03-skills.md](./03-skills.md) の食文化）。
...
conceive はアクションではない。親密度条件から自動発生するイベントになった（[05-kinship.md](./05-kinship.md)）。
...
- [x] `proposal` の定義：**P0-1 で削除に決定**。event として運ぶと無料シグナルになるため。申し出（teach 等）は give プロトコルで brain が自作する（[06-communication.md](./06-communication.md)）。
...
- [ ] 空間モデルの詳細（[90-open-questions.md](./90-open-questions.md) #15）：体積の可視性、空間満杯時の harvest / 受領 / invoke 産出の挙動（action-failed か自動 discard か）、身体体積の年齢関数、`space-free` を公開してよいか（無料の全体情報になる）。
