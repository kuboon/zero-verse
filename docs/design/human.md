...
| --- | --- | --- | --- |
| **health**（体力） | 生存そのもの。0 で死。**何もしなければ毎月減少**し、出産・思考（fuel）でも低下する | 最大値が加齢で単調に低下 | resource を消費する skill（食事）で回復（[03-skills.md](./03-skills.md)） |
...
- 未占有空間が無ければ保有体積を増やせない（harvest・受領・invoke の産出が制限される）。溜め込みは g 減衰に加えて空間でも課金される。
- 食事などの消費は廃棄物 resource を生み、**discard するまで空間を圧迫し続ける**。ゴミ処理が生活コストになる。
