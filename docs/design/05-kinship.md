...
- **発生条件**：以下がすべて揃った月に conceive が発生する。
  ...
  4. 刷り込みペアでない（下記）
  5. 女性側が妊娠中でない
- 帰結：**求愛 = 親密度への投資**。give や teach で親密度を集中させることが配偶行動になり、「互いの親密度ポートフォリオの過半を占め合う」という定義からして排他的な関係だけが子をなす。婚姻契約の if-intimacy（[06-communication.md](./06-communication.md)）は、この 50% 集中を守る装置としてそのまま接続する。
...
- [x] conceive の成立方式：**再決定**。アクションを廃止し、相対親密度（相互 50% 超）＋異性＋fertility 窓＋非刷り込みからの自動発生（[90-open-questions.md](./90-open-questions.md) #1）。
...
- [ ] 刷り込みの閾値と判定タイミング、相対親密度 50% という値の妥当性（world パラメータ化するか）（[90-open-questions.md](./90-open-questions.md) #10）。
