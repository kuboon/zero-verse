...
| **M4** 血縁 | 血縁構造が投資行動に現れるか | 継承・conceive・relation-hint |
...
- conceive（相互指定）、出生、baby brain（0〜6歳の world 提供共通 brain）、6歳での 50/50 brain 継承（切替は観測不能）。
- relation-hint（self-child / self-parent / spouse / unknown）と child-born イベント。
- memory-limit の年齢関数（老化による物忘れ）、能力曲線の完成。
- 参照 brain：家族投資 brain（子に無償で teach する）。

### 合格基準

- 血縁度に応じた投資の傾斜（子への teach 頻度が非血縁への teach 頻度を上回る等）が、world 側に家族概念を書かずに観測される。
...
| 1 | conceive に spouse 状態を挟むか | **M4** | まず相互指定のみ（完全創発）で試す |
